...
public class ClickDragScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (Input.GetMouseButtonUp(0))
        {
            ...
            // Interaction with GridManager
            Vector2 tileIndex = currentlyDraggedObject.gameObject.GetComponent<NavigationObject>().GetGridIndex();
            if (currentlyDraggedObject.gameObject.CompareTag("Mines")) // We've released a mines tile.
            {
            ...
            else if (currentlyDraggedObject.gameObject.CompareTag("Ship")) // We've released a ship tile.
            {
                GridManager.Instance.GetGrid()[(int)tileIndex.y, (int)tileIndex.x].GetComponent<TileScript>().SetStatus(TileStatus.START);
            }
            else if (currentlyDraggedObject.gameObject.CompareTag("Planet")) // We've released a planet tile.
            {
