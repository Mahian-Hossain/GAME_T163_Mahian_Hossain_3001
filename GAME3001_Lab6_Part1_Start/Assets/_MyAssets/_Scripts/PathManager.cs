...
public class PathManager : MonoBehaviour
{
    ...
    public void GetShortestPath(PathNode start, PathNode goal)
    {
        if (path.Count > 0)
        {
            // GridManager.Instance.SetTileStatuses(); // This line is commented out as it seems to be unnecessary and may cause an error.
        ...
        while (openList.Count > 0)
        {
            ...
            if (currentRecord.Node == goal)
            {
                ...
                // currentRecord.Node.Tile.GetComponent<TileScript>().SetStatus(TileStatus.CLOSED); // This line is commented out as it seems to be unnecessary and may cause an error.
                break;
            }
            ...
            for (int i = 0; i < connections.Count; i++)
            {
                ...
                if (!ContainsNode(openList, endNode))
                {
                    ...
                    // endNodeRecord.Node.Tile.GetComponent<TileScript>().SetStatus(TileStatus.CLOSED); // This line is commented out as it seems to be unnecessary and may cause an error.
                }
            }
            openList.Remove(currentRecord);
            closedList.Add(currentRecord);
        ...
        else
        {
            ...
            while (currentRecord.Node != start)
            {
                ...
                // currentRecord.Node.Tile.GetComponent<TileScript>().SetStatus(TileStatus.PATH); // This line is commented out as it seems to be unnecessary and may cause an error.
